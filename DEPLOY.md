# Deploying DGRINGO to Hostinger (GitHub → Node.js app)

One Node server hosts **everything** from a single deploy:

| URL | What |
|-----|------|
| `https://dgringo.com/`        | Marketing website (opens directly) |
| `https://dgringo.com/app`     | The DGRINGO app |
| `https://dgringo.com/admin`   | Control Hub (admin) |
| `https://dgringo.com/api/*`   | Telnyx + PayPal proxy (secret key injected server-side) |
| `https://dgringo.com/webhooks/telnyx` | Telnyx inbound-SMS / status webhook |
...
The repo is `https://github.com/khanbro2/DGRingo`. Secrets are **never** committed
...
2. Click **Create application** and set:
   ...
   - **Application root:** e.g. `domains/dgringo.com/app` (any empty folder)
   - **Application URL:** your domain `dgringo.com`
   - **Startup file:** `server/telnyx-proxy.mjs`
3. **Connect the GitHub repo** `khanbro2/DGRingo` (branch `main`), **or** upload the files.
...
2. Make sure **Application URL** is your domain so `dgringo.com` maps to this app.
3. Visit `https://dgringo.com` — the marketing site should load immediately.
...
```
https://dgringo.com/webhooks/telnyx
```
